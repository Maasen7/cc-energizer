local e=peripheral.find("BigReactors-Energizer")
local m=peripheral.find("monitor")
m.setTextScale(2)

local units={FE=1,KFE=1e3,MFE=1e6,GFE=1e9,TFE=1e12,PFE=1e15,EFE=1e18}

local function parseFE(t)
 local n,u=string.match(t,"([%d%.]+)%s*(%a+)")
 return (tonumber(n) or 0)*(units[u] or 1)
end

local function at(x,y,t,fg,bg)
 m.setCursorPos(x,y)
 m.setTextColor(fg or colors.black)
 m.setBackgroundColor(bg or colors.white)
 m.write(t)
end

local function center(y,t,fg,bg)
 local w,_=m.getSize()
 at(math.floor((w-#t)/2)+1,y,t,fg,bg)
end

local function fillLine(y,bg)
 local w,_=m.getSize()
 at(1,y,string.rep(" ",w),colors.black,bg)
end

local function hline(y)
 local w,_=m.getSize()
 at(2,y,string.rep("-",w-2),colors.blue,colors.white)
end

local function sci(n)
 if n>=1e12 then return string.format("%.2fT",n/1e12) end
 if n>=1e9 then return string.format("%.2fG",n/1e9) end
 if n>=1e6 then return string.format("%.2fM",n/1e6) end
 if n>=1e3 then return string.format("%.2fK",n/1e3) end
 return tostring(math.floor(n))
end

local function bar(y,pct)
 local w,_=m.getSize()
 local bw=w-10
 local x=6
 local fill=math.floor(bw*pct)

 at(x-1,y,"[",colors.black,colors.white)
 for i=1,bw do
  at(x+i-1,y," ",colors.white,i<=fill and colors.lime or colors.lightGray)
 end
 at(x+bw,y,"]",colors.black,colors.white)
end

while true do
 m.setBackgroundColor(colors.white)
 m.clear()

 local storedText=e.getEnergyStoredAsText()
 local storedFE=parseFE(storedText)
 local capFE=e.getEnergyCapacity()
 local pct=math.min(storedFE/capFE,1)
 local input=e.getEnergyInsertedLastTick()
 local output=e.getEnergyExtractedLastTick()
 local on=e.getActive()
 local w,h=m.getSize()

 center(1,"ENERGIZER",colors.blue,colors.white)
 hline(2)

 fillLine(3,colors.black)
 center(3,"STATUS: "..(on and "ONLINE" or "OFFLINE"),on and colors.lime or colors.red,colors.black)

 center(5,string.format("%.0f%%",pct*100),colors.black,colors.white)
 bar(7,pct)
 center(8,storedText.." / "..sci(capFE).." FE",colors.gray,colors.white)

 local mid=math.floor(w/2)
 at(mid,10,"|",colors.blue,colors.white)
 at(mid,11,"|",colors.blue,colors.white)
 at(mid,12,"|",colors.blue,colors.white)

 at(5,10,"IN ->",colors.blue,colors.white)
 at(5,11,tostring(input),colors.blue,colors.white)
 at(5,12,"FE/t",colors.gray,colors.white)

 local ox=w-14
 at(ox,10,"OUT ->",colors.orange,colors.white)
 at(ox,11,tostring(output),colors.orange,colors.white)
 at(ox,12,"FE/t",colors.gray,colors.white)

 fillLine(h,colors.black)
 center(h,"TAP ANYWHERE TO TOGGLE",colors.white,colors.black)

 local timer=os.startTimer(0.5)
 while true do
  local ev,_,x,y=os.pullEvent()
  if ev=="timer" then break end
  if ev=="monitor_touch" then
   if x<=2 and y<=1 then return end
   e.setActive(not on)
   break
  end
 end
end
